...
class DriverPromotion(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    active = db.Column(db.Boolean, default=True, nullable=False)

    title = db.Column(db.String(160), default="Promoção para motoristas", nullable=False)
    subtitle = db.Column(
        db.String(220),
        default="Condições especiais para quem roda todos os dias",
        nullable=False
    )

    daily_price = db.Column(db.Numeric(10, 2), default=0, nullable=False)
    daily_note = db.Column(db.String(180), default="Valor especial do dia", nullable=False)

    weekly_price = db.Column(db.Numeric(10, 2), default=0, nullable=False)
    weekly_washes = db.Column(db.Integer, default=0, nullable=False)

    monthly_price = db.Column(db.Numeric(10, 2), default=0, nullable=False)
    monthly_washes = db.Column(db.Integer, default=0, nullable=False)

    audience = db.Column(
        db.String(180),
        default="Motoristas de app, táxi e particular",
        nullable=False
    )
    whatsapp = db.Column(db.String(30), default="+55 15 99665-4845", nullable=False)
...
def home():
    ...
    driver_promo = DriverPromotion.query.first()
    return render_template("index.html", services=services, driver_promo=driver_promo)
...
def admin_driver_promotion():
    promo = DriverPromotion.query.first()
    if not promo:
        promo = DriverPromotion()
        db.session.add(promo)
        db.session.commit()

    if request.method == "POST":
        promo.title = request.form.get("title", "").strip() or "Promoção para motoristas"
        promo.subtitle = request.form.get("subtitle", "").strip() or "Condições especiais para quem roda todos os dias"
        promo.audience = request.form.get("audience", "").strip() or "Motoristas de app, táxi e particular"
        promo.daily_note = request.form.get("daily_note", "").strip() or "Valor especial do dia"
        promo.whatsapp = request.form.get("whatsapp", "").strip() or "+55 15 99665-4845"

        promo.daily_price = max(Decimal("0"), decimal_from_form(request.form.get("daily_price", "0")))
        promo.weekly_price = max(Decimal("0"), decimal_from_form(request.form.get("weekly_price", "0")))
        promo.monthly_price = max(Decimal("0"), decimal_from_form(request.form.get("monthly_price", "0")))

        try:
            promo.weekly_washes = max(0, int(request.form.get("weekly_washes", "0") or 0))
            promo.monthly_washes = max(0, int(request.form.get("monthly_washes", "0") or 0))
        except ValueError:
            promo.weekly_washes = 0
            promo.monthly_washes = 0

        promo.active = request.form.get("active") == "on"

        db.session.commit()
        flash("Promoção para motoristas atualizada no site.", "success")
...
def seed():
    ...
    if DriverPromotion.query.count() == 0:
        db.session.add(DriverPromotion(
            active=True,
            ...
            daily_price=0,
            daily_note="Valor especial do dia",
            weekly_price=0,
            weekly_washes=0,
            monthly_price=0,
            monthly_washes=0,
