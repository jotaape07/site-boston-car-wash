...
PROMO_PREFIX = "__PROMO_DRIVER_"
PROMO_DAILY = "__PROMO_DRIVER_DAILY"
PROMO_WEEKLY = "__PROMO_DRIVER_WEEKLY"
PROMO_MONTHLY = "__PROMO_DRIVER_MONTHLY"
PROMO_META = "__PROMO_DRIVER_META"


def _promo_service(name):
    return Service.query.filter_by(name=name).first()


def _ensure_driver_promo_rows():
    defaults = [
        (PROMO_DAILY, "Valor especial do dia", Decimal("0"), 0),
        (PROMO_WEEKLY, "Pacote semanal", Decimal("0"), 0),
        (PROMO_MONTHLY, "Pacote mensal", Decimal("0"), 0),
        (PROMO_META, json.dumps({
            "active": True,
            "title": "Promoção para motoristas",
            "subtitle": "Condições especiais para quem roda todos os dias",
            "audience": "Motoristas de app, táxi e particular",
            "whatsapp": "+55 15 99665-4845"
        }, ensure_ascii=False), Decimal("0"), 0),
    ]

    changed = False
    for name, description, price, duration in defaults:
        if not _promo_service(name):
...
def get_driver_promo():
    daily = _promo_service(PROMO_DAILY)
    weekly = _promo_service(PROMO_WEEKLY)
    monthly = _promo_service(PROMO_MONTHLY)
    meta_row = _promo_service(PROMO_META)

    if not any([daily, weekly, monthly, meta_row]):
        return None

    meta = {}
    if meta_row and meta_row.description:
        try:
            meta = json.loads(meta_row.description)
        except Exception:
            meta = {}

    return {
        "active": bool(meta.get("active", True)),
        "title": meta.get("title", "Promoção para motoristas"),
        "subtitle": meta.get("subtitle", "Condições especiais para quem roda todos os dias"),
        "audience": meta.get("audience", "Motoristas de app, táxi e particular"),
        "whatsapp": meta.get("whatsapp", "+55 15 99665-4845"),
        "daily_price": Decimal(daily.price or 0) if daily else Decimal("0"),
        "daily_note": (daily.description if daily and daily.description else "Valor especial do dia"),
        "weekly_price": Decimal(weekly.price or 0) if weekly else Decimal("0"),
        "weekly_washes": int(weekly.duration_minutes or 0) if weekly else 0,
        "monthly_price": Decimal(monthly.price or 0) if monthly else Decimal("0"),
        "monthly_washes": int(monthly.duration_minutes or 0) if monthly else 0,
    }


@app.route("/")
def home():
    _ensure_driver_promo_rows()
    services = Service.query.filter(
        Service.active.is_(True),
        ~Service.name.startswith(PROMO_PREFIX)
    ).order_by(Service.name.asc()).all()
    return render_template(
        "index.html",
        services=services,
        driver_promo=get_driver_promo()
    )
...
def admin_dashboard():
    ...
    completed_today = Appointment.query.filter(
        ...
        db.func.date(Appointment.completed_at) == today
    ).count()
...
def admin_cash():
    ...
    completed_services = Appointment.query.filter(
        Appointment.status == "concluido",
        Appointment.completed_at.isnot(None),
        db.func.date(Appointment.completed_at) == selected_date
...
@app.route("/admin/promocao-motoristas", methods=["GET", "POST"])
@admin_required
def admin_driver_promotion():
    _ensure_driver_promo_rows()
    ...
    meta = {}
    try:
        meta = json.loads(meta_row.description or "{}")
    except Exception:
        meta = {}

    if request.method == "POST":
        daily.price = max(Decimal("0"), decimal_from_form(request.form.get("daily_price", "0")))
        daily.description = request.form.get("daily_note", "").strip() or "Valor especial do dia"

        weekly.price = max(Decimal("0"), decimal_from_form(request.form.get("weekly_price", "0")))
        monthly.price = max(Decimal("0"), decimal_from_form(request.form.get("monthly_price", "0")))

        try:
            weekly.duration_minutes = max(0, int(request.form.get("weekly_washes", "0") or 0))
            monthly.duration_minutes = max(0, int(request.form.get("monthly_washes", "0") or 0))
        except ValueError:
            flash("Confira a quantidade de lavagens.", "danger")
            return redirect(url_for("admin_driver_promotion"))

        meta = {
            "active": request.form.get("active") == "on",
            "title": request.form.get("title", "").strip() or "Promoção para motoristas",
            "subtitle": request.form.get("subtitle", "").strip() or "Condições especiais para quem roda todos os dias",
            "audience": request.form.get("audience", "").strip() or "Motoristas de app, táxi e particular",
            "whatsapp": request.form.get("whatsapp", "").strip() or "+55 15 99665-4845",
        }
        meta_row.description = json.dumps(meta, ensure_ascii=False)

        db.session.commit()
        flash("Promoção atualizada no site.", "success")
        return redirect(url_for("admin_driver_promotion"))

    promo = get_driver_promo()
    return render_template("admin_driver_promotion.html", promo=promo)
...
def seed():
    ...
    def ensure_admin(email, password, name="Administrador"):
    ...
    ensure_admin(
        os.getenv("ADMIN_EMAIL", "admin@bostoncarwash.local"),
        os.getenv("ADMIN_PASSWORD", "TroqueEssaSenha123!"),
        "Administrador"
    )

    # Segundo administrador opcional (ex.: pai / responsável)
    ensure_admin(
        os.getenv("ADMIN2_EMAIL", ""),
        os.getenv("ADMIN2_PASSWORD", ""),
        os.getenv("ADMIN2_NAME", "Administrador 2")
    )
    ...
    # Garante que a promoção para motoristas já exista no primeiro acesso.
    _ensure_driver_promo_rows()
